...
(function () {
    ...
    var reCustomAPI = new RegExp(/\(customapi\s([\w\-\.\_\~\:\/\?\#\[\]\@\!\$\&\'\(\)\*\+\,\;\=]+)\)/), // URL[1]
        ...
        tagCheck = new RegExp(/\(help=|\(views\)|\(subscribers\)|\(age\)|\(sender\)|\(@sender\)|\(baresender\)|\(random\)|\(1\)|\(2\)|\(3\)|\(count\)|\(pointname\)|\(points\)|\(currenttime|\(price\)|\(#|\(uptime\)|\(follows\)|\(game\)|\(status\)|\(touser\)|\(echo\)|\(alert [,.\w]+\)|\(readfile|\(readfileline|\(1=|\(countdown=|\(countup=|\(downtime\)|\(pay\)|\(onlineonly\)|\(offlineonly\)|\(code=|\(followage\)|\(followdate\)|\(hours\)|\(gameinfo\)|\(titleinfo\)|\(gameonly=|\(useronly=|\(playtime\)|\(gamesplayed\)|\(pointtouser\)|\(lasttip\)|\(writefile .+\)|\(readfilerand|\(team_|\(commandcostlist\)|\(playsound |\(customapi |\(customapijson /),
        customCommands = [],
        ...
        if ($.bot.isModuleEnabled('./custom/customCommands.js')) {
            ...
            for (i in aliases) {
                if (!$.commandExists(aliases[i])) {
                    $.registerChatCommand('./custom/customCommands.js', aliases[i], $.getIniDbNumber('permcom', aliases[i], 7));
    ...
    $.bind('command', function (event) {
        ...
        if (command.equalsIgnoreCase('addcom')) {
            ...
            $.registerChatCommand('./custom/customCommands.js', action);
        ...
        if (command.equalsIgnoreCase('editcom')) {
            ...
            $.registerChatCommand('./custom/customCommands.js', action, 7);
            $.inidb.set('command', action, argsString);
            customCommands[action] = argsString;
            return;
        }
        ...
        if (command.equalsIgnoreCase('enablecom')) {
            ...
            $.registerChatCommand(($.inidb.exists('tempDisabledCommandScript', action) ? $.inidb.get('tempDisabledCommandScript', action) : './custom/customCommands.js'), action);
            return;
        }
        ...
        if (command.equalsIgnoreCase('aliaslist')) {
            ...
            $.consoleLn(aliaslistDbTableName); // DEBUG
            const spreadsheetId = $.getIniDbString(aliaslistDbTableName, 'spreadsheetId', undefined);
            $.consoleLn(spreadsheetId); // DEBUG
    ...
    $.bind('initReady', function () {
        $.registerChatCommand('./custom/customCommands.js', 'addcom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'pricecom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'paycom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'aliascom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'delalias', 2);
        $.registerChatCommand('./custom/customCommands.js', 'delcom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'editcom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'tokencom', 2);
        $.registerChatCommand('./custom/customCommands.js', 'permcom', 1);
        $.registerChatCommand('./custom/customCommands.js', 'commands', 7);
        $.registerChatCommand('./custom/customCommands.js', 'disablecom', 1);
        $.registerChatCommand('./custom/customCommands.js', 'enablecom', 1);
        $.registerChatCommand('./custom/customCommands.js', 'botcommands', 2);
        $.registerChatCommand('./custom/customCommands.js', 'resetcom', 2);

        // Custom commands
        $.registerChatCommand('./custom/customCommands.js', 'aliaslist');
    });
    ...
    $.bind('webPanelSocketUpdate', function (event) {
        if (event.getScript().equalsIgnoreCase('./custom/customCommands.js')) {
            if (event.getArgs()[0] == 'remove') {
                ...
                $.registerChatCommand('./custom/customCommands.js', event.getArgs()[1].toLowerCase());
